...
def feat_merge(opt, cnt_feats, sty_feats, start_step=0, layer_weights=None):
    N = opt.save_feat_steps
    gamma_start = opt.gamma_start
    gamma_end = opt.gamma_end
    gammas = [
        gamma_end + 0.5 * (gamma_start - gamma_end) * (1 + np.cos(np.pi * i / (N - 1)))
        for i in range(N)
    ]
    feat_maps = [{'config': {
        'gamma': gammas[i],
        ...
        for ori_key in ori_keys:
            ...
            if ori_key[-1] in ('k', 'v'):
                if layer_weights is not None:
                    # key format: e.g. "output_block_7_self_attn_k"
                    parts = ori_key.split('_')
                    layer_idx = int(parts[2])
                    w = layer_weights.get(layer_idx, 1.0)
                    # blend content & style k/v by w
                    feat_maps[i][ori_key] = cnt_feat[ori_key] * (1.0 - w) + sty_feat[ori_key] * w
                else:
...
def main():
    ...
    parser.add_argument("--attn_layer", type=str, default='3,4,5,6,7,8,9,10,11', help='injection attention feature layers')
    ...
    parser.add_argument('--output_path', type=str, default='output-adaptive-g-T15')
    parser.add_argument("--without_init_adain", action='store_true')
    parser.add_argument("--without_attn_injection", action='store_true')

    parser.add_argument('--gamma_end', type=float, default=0.45, help = 'final query-preservation at last step (lower = more style)')
    parser.add_argument('--gamma_start', type=float, default=1.0, help = 'initial query-preservation at first step (1.0 = no style)')
    parser.add_argument('--layer_weight', type=float, default=0.6,  help='linear layer weight initialization')

    opt = parser.parse_args()

    min_layer, max_layer = 3, 11
    layer_weights = {
            l: opt.layer_weight + (1- opt.layer_weight) * ((l - min_layer) / (max_layer - min_layer)) for l in range(min_layer, max_layer + 1)}
    ...
    N = opt.save_feat_steps
    gamma_start = opt.gamma_start
    gamma_end = opt.gamma_end  # you can set this via a new --gamma_end flag
    gammas = [
        gamma_end + 0.5 * (gamma_start - gamma_end) * (1 + np.cos(np.pi * i / (N - 1)))
        for i in range(N)
    ]
    ...
    feat_maps = [{'config': {
        'gamma': gammas[i],
        'T': opt.T
    }} for i in range(opt.save_feat_steps)]
            ...
            with torch.no_grad():
                with precision_scope("cuda"):
                    with model.ema_scope():
                        ...
                        feat_maps = feat_merge(opt, cnt_feat, sty_feat, start_step=start_step, layer_weights=layer_weights)
